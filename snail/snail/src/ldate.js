    ...
    if (/(y+)/.test(fmt)) {
        ...
        for (var key in style) {
            if (new RegExp("(" + key + ")").test(fmt)) {
                fmt = fmt.replace(RegExp.$1, (RegExp.$1.length == 1) ? style[key] : ("00" + style[key]).substr(("" + style[key]).length));
            }
        }
    }
...
Date.VerifyDateString = function (string) {
    ...
    return date.getFullYear() == r[1] && (date.getMonth()) == r[2] && date.getDate() == r[3] && date.getHours() == r[4] && date.getMinutes() == r[5] && date.getSeconds() == r[6];
};
